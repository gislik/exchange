...
import Test.Hspec
import Test.QuickCheck (chooseAny, forAll, property, suchThat)
...
main = hspec $ do
  describe "Amount" $ do
    ...
    it "should be instances of num" $
      forAll (chooseAny `suchThat` \(x, y) -> x >= 0 && y >= 0 && x >= y) $ do
        -- \x y -> do
        \(x, y) -> do
          (toAmount x) + (toAmount y) `shouldBe` ((toAmount $ x + y) :: Amount BTC)
          (toAmount x) - (toAmount y) `shouldBe` ((toAmount $ x - y) :: Amount BTC)

    it "should be instances of eq" $
      forAll (chooseAny `suchThat` \(x, y) -> x /= y) $ do
        \(x, y) -> do
  ...
  describe "Exchange" $ do
    ...
    context "when an order is canceled" $ do
      ...
      it "should be removed" $ do
        ...
        Book.bids book'
          `shouldBe` [ Order.Maker (Order.limit Bid (Time 0) (toAmount 2) BTC (toPrice 10) USD)
                     ]
